  ...
  output$summary <- renderInfoBox({
    if(exact_percentile<5){
      summary <- infoBox(tags$h3("Overall"),
                         tags$p("Slightly skinny!", tags$br(),formatC(target_weight-target_weight, digits=1, format="f"), 
                                "kg less than the healthly weight range!",
    ...
    if(85<=exact_percentile&&exact_percentile<95){
      summary <- infoBox(tags$h3("Overall"),
                         tags$p("Only", formatC(weight-target_weight, digits=1, format="f"), "kg from the healthly weight range!",
                                style = "font-size: 150%; font-weight: bold;"),
